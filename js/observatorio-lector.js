(() => {
  ...
  const dataPath = mode === 'base' ? '/data/normativa/ai-act-full.json' : '/data/normativa/ai-act-changes.json';
  ...
  function renderBase(data){
    ...
    $('#reader-source').href=data.source.html_url;
    renderSummary([
  ...
  fetch(dataPath,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('No se pudo cargar la norma');return r.json();}).then(data=>mode==='base'?renderBase(data):renderChanges(data)).catch(err=>{$('#reader-content').innerHTML=`<div class="reader-error">${esc(err.message)}</div>`;});
})();
